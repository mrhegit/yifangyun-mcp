...
`1.0.0-beta.8`（`contract_version=3`）使用轻量 Drive 平面与可选的 Workspace、Inventory、Evidence、Organization 平面。普通发现工具保持低成本；范围证明、完整性结论和原件固化必须进入对应的受约束工具。

## 通用结果契约

成功结果的完整对象位于 `structuredContent`。当文本序列化超过 12,000 字符时，`content` 返回 `compact_preview`，其中保留真实样本、遗漏计数、Ref、路径、分页和 `next_action` 等操作锚点；优先级字段（如 `agent_warnings`、`must_release`、`content_delivery`、`claim_allowed`、`agent_guidance`）会尽量保留。
...
- `YFY_CURSOR_INVALID` / `YFY_INVENTORY_CURSOR_INVALID`：输入无效，使用 `first_request` 重启。公开错误路径 `error.diagnostics.reason` 为 `not_base64url` | `envelope_invalid` | `signature_invalid` | `payload_invalid`，**不回传 raw 解码文本**。
...
- `server.version`、`contract_version`（beta.8 为 `3`）、`build_id`、`build_commit`、`instance_id`、`started_at`
...
| `yfy_search` | `request.query/in/kind/field/detail/exact_name/sort/direction/limit/include_unverified_index_hits/access_context` | `agent_warnings/hits/unverified_hits/coverage/page` |
...
| `yfy_versions` | `request.file/limit` | 文件回显、版本（`ref` 可 null）、`version_selection_rules`、fingerprint 和分页 |
...
- `include_unverified_index_hits=true` 时，verified 与 unverified 按 Provider 返回顺序组成一个受同一 `limit`/cursor 约束的候选流；切片后两类分别投影到 `hits` 和 `unverified_hits`，`page.returned_count` 等于两者本页数量之和。
...
- `inventory_id`（日志和显示用 UUID；调用工具仍使用完整签名 `inventory` Ref）
...
- `agent_guidance.may_claim_absence`、可选 `absence_forbidden_reason`、`recommended_actions`
...
省略 `query` 表示列出。默认 kind 为 all、match fields 为 name+path、大小写不敏感、limit 为 25，最大 100。Search 每页重复返回 `workspace`、`scan_root`、`agent_guidance` 和 `completeness`，因此子树缺失结论始终绑定具体 scan root。`view.commit_watermark` 是当前分页视图固定的水位，`current_commit_watermark` 是任务最新进度；两者不同时，继续翻页仍保持旧视图稳定，新建 first_request 才看到新提交。
...
这些 toolset 不默认启用。读取内容优先使用 `yfy_open`，不要为方便而启用 `transfer`；投标默认矩阵 **永不包含 transfer**。范围内原件固化使用 `yfy_capture`。
