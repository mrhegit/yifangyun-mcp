...
import { normalizeFileVersions, selectFileVersion, versionSelectionProof, type VersionSelector } from "../domain/fileVersions.js";
import { YifangyunError } from "../client.js";
import type { AppRuntime } from "../runtime/runtime.js";
import { registerTool } from "./tooling.js";
import { ProvenanceSchema, VersionSelectionProofSchema, VersionSelectorSchema } from "./schemas.js";
...
export function registerTransferTools(server: McpServer, runtime: AppRuntime): void {
  ...
  registerTool(server, "yfy_transfer_ticket_get", {
    ...
    inputSchema: { file_id: z.string().regex(/^\d+$/), version: VersionSelectorSchema.default({ kind: "current" }), access_context: z.string().trim().min(1).optional() },
    outputSchema: { download_url: z.string().url(), selection: VersionSelectionProofSchema, validation_level: z.literal("selector_prevalidated"), sensitive: z.boolean(), expires_quickly: z.boolean(), provenance: z.array(ProvenanceSchema) }
  }, { readOnly: true }, async ({ file_id, version, access_context }, extra) => {
    ...
    const selected = selectFileVersion(normalizeFileVersions(versionsResponse.data).versions, selector);
    const response = await runtime.gateway.getUser(`/v2/file/${encodeURIComponent(String(file_id))}/download_v2`, resolved.context.id, {
      version: selected.download_version,
    ...
    return { download_url: data.download_url, selection: versionSelectionProof(selected, selector, "selector_prevalidated"), validation_level: "selector_prevalidated", sensitive: true, expires_quickly: true, provenance: [provenance(versionsResponse.meta, resolved.context.id), provenance(response.meta, resolved.context.id)] };
  });
}
