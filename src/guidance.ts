...
export function serverInstructions(runtime: AppRuntime): string {
  ...
  if (runtime.config.toolsets.includes("evidence") && runtime.config.authorityScopes.length > 0) {
    instructions.push(
      "Use yfy_capture only when workspace-bound original bytes are required. It may embed a verified small-text MCP resource when include_text_preview=true; inspect content_delivery (agent_readable) and always honor must_release. Execute next_action (yfy_resource_release) when finished; binary modes set agent_readable=false and do not prove the model read the body."
    );
  }
  if (runtime.config.toolsets.includes("transfer")) {
    instructions.push("Do not use yfy_transfer_ticket_get as an evidence or ordinary read path (usage_policy=special_integration_only, not_for_evidence); prefer yfy_open/yfy_capture. Transfer URLs are sensitive (do_not_echo_url) and must not be logged.");
  }
...
export function registerGuidance(server: McpServer, runtime: AppRuntime): void {
  server.registerResource("yfy_overview", "yfy://guide/overview", {
    ...
    ...(runtime.config.toolsets.includes("drive") ? [
      ...
      "3. Existing FileRef and bytes needed without a Workspace claim: call `yfy_open`, inspect `content_delivery.agent_readable` / whether a verified text resource was embedded or `resources/read` is required, then execute `next_action` (`yfy_resource_release`).",
  ...
  server.registerResource("yfy_safety", "yfy://guide/safety", {
    ...
    "- `yfy_open` / `yfy_capture` may include a standard embedded text resource; inspect `content_delivery.agent_readable` / `model_has_body_text`, do not assume resource links were fetched, and always honor `must_release` plus `next_action` (`yfy_resource_release`).",
    ...
    "- Release content resources after use via `next_action` / `yfy_resource_release` (idempotent).",
    "- Never treat transfer tickets as an evidence path (`usage_policy=special_integration_only`, `not_for_evidence`); prefer `yfy_open`/`yfy_capture`. Transfer URLs are sensitive (`do_not_echo_url`) and must not be logged.",
